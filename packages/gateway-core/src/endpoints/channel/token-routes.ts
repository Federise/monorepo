...
export function registerTokenChannelRoutes(app: Hono<{ Variables: GatewayEnv }>) {
  ...
  app.use("/channel/read", async (c, next) => {
    ...
    try {
      ...
      return c.json({ code: "INVALID_REQUEST", message: "Invalid JSON body" }, 400);
    }

    const { channelId, afterSeq = 0, limit = 50 } = body;

    if (!channelId) {
      return c.json({ code: "INVALID_REQUEST", message: "Missing channelId" }, 400);
    }
  ...
  app.use("/channel/append", async (c, next) => {
    ...
    if (!channelId || !content) {
      return c.json({ code: "INVALID_REQUEST", message: "Missing channelId or content" }, 400);
    }
  ...
  app.use("/channel/delete-event", async (c, next) => {
    ...
    if (!channelId || targetSeq === undefined) {
      return c.json({ code: "INVALID_REQUEST", message: "Missing channelId or targetSeq" }, 400);
    }
    ...
    if (!hasDeleteAny && targetEvent.authorId !== verified.authorId) {
      return c.json({ code: "FORBIDDEN", message: "Can only delete own events" }, 403);
    }
